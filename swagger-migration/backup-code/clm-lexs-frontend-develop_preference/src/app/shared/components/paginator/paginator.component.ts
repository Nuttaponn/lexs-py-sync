...
export class PaginatorComponent extends _MatPaginatorMixinBase implements OnInit, HasInitialized {
  ...
  private _updatePageLabels(start: number = 1, end: number = 7) {
    if (!this._length || !this._pageSize) {
      // handle when length === 0
      this.pages = [{ pageNumber: 1, label: '1', disabled: false }];
      return;
    }
  ...
  generatePageLabels = (total_page: number, each_side: number, curr_page: number) => {
    ...
    if (start_page > 1) pages.push('1');

    if (start_page > 2) pages.push('...');

    for (let x = start_page; x <= end_page; x++) pages.push('' + x);

    if (end_page < total_page - 1) pages.push('...');

    if (end_page < total_page) pages.push('' + total_page);
